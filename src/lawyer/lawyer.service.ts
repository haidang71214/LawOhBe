...
export class LawyerService {
  ...
  constructor(
    ...
    @InjectModel(SubTypeLawyer.name)
    private SubTypeLawyerModel: Model<SubTypeLawyer>,
    @InjectModel(CustomPrice.name)
    private CustomeerPriceModel: Model<CustomPrice>,
  ) {}
  ...
  async update(updateLawyerDto: UpdateLawyerDto, userId: string) {
    try {
      ...
      if (thisLawyer?.role === 'lawyer') {
        ...
        if (
          thisLawyer.typeLawyer === null ||
          thisLawyer.typeLawyer === undefined
        ) {
          ...
          // mình đỉnh vl
          await this.TypeLawyerModel.replaceOne(
            { _id: idTypeLawyer },
            {
              type: type_lawyer,
          ...
          const findTypeForLawyer =
            await this.TypeLawyerModel.findById(idTypeLawyer);
          if (
            !findTypeForLawyer ||
            !findTypeForLawyer.type ||
            !Array.isArray(findTypeForLawyer.type)
          ) {
            throw new BadRequestException(
              'Không tìm thấy thông tin chuyên môn.',
            );
          }
          ...
          const typesToRemove = existingTypes.filter(
            (type) => !typesProcessed.includes(type),
          );
  ...
  async updateLawyer(
    ...
    try {
      ...
      throw new BadRequestException(
        `Lỗi khi cập nhật luật sư: ${error.message}`,
      );
    }
  }
  ...
  async filterLawyers(
    ...
    if (typeLawyer) {
      const typeLawyers = await this.TypeLawyerModel.find({
        type: { $regex: typeLawyer, $options: 'i' },
      })
        .select('lawyer_id')
        .exec();
  ...
  async getDetailLawyer(id: string) {
    try {
      ...
      const data = await this.UserModel.findById(id).populate({
