...
export class PriceRangeService {
  constructor(
    ...
    private MarketPriceRangeModel: Model<MarketPriceRange>,
  ...
  async setLawyerCustomPrice(userId: string, body: CustomPriceRangeDto) {
    try {
      ...
      if (checkLaywer?.role === 'lawyer') {
        ...
        if (
          checkPrice &&
          price >= checkPrice.minPrice &&
          price <= checkPrice.maxPrice
        ) {
          const checkGiaTonTai = await this.CustomPriceModel.findOne({
            lawyer_id: userId,
            type: Type,
          });
          if (checkGiaTonTai) {
            // nếu tồn tại thì thay đổi
            await this.CustomPriceModel.replaceOne(
              { _id: checkGiaTonTai._id },
              {
                lawyer_id: userId,
                type: Type,
                price,
  ...
  async update(
    ...
    try {
      ...
      const customPrices = await this.CustomPriceModel.find({
        type: normalizedType,
      });
        ...
        if (item.price > updatePriceRangeDto.maxPrice) {
          ...
          for (const booking of bookings) {
            if (booking.booking_start && booking.booking_end) {
              const days =
                (booking.booking_end.getTime() -
                  booking.booking_start.getTime()) /
                (1000 * 60 * 60 * 24);
