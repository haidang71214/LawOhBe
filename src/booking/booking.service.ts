...
export class BookingService {
  constructor(
    ...
    @InjectModel(MarketPriceRange.name)
    private MarketPriceRangeModel: Model<MarketPriceRange>, // db set cứng khoảng giá
  ...
  async acceptBooking(lawyerid: string, client_id: string, booking_id: string) {
    ...
    try {
      ...
      if (check) {
        ...
        const bookingData = await this.BookingModel.findOneAndUpdate(
          { client_id, lawyer_id: lawyerid, _id: booking_id },
          {
            status: 'accept',
          },
        );
        ...
        const clientData = await this.UserModel.findById(
          bookingData?.client_id,
        );
        const lawyerData = await this.UserModel.findById(
          bookingData?.lawyer_id,
        );
        // to, tittle, content
  ...
  async cancelBooking(userId: string, id: string) {
    try {
      const checkStatus = await this.BookingModel.findOne({
        client_id: userId,
        _id: id,
      });
      if (
        checkStatus?.status === 'accept' ||
        checkStatus?.status === 'reject'
      ) {
        return {
          status: 409,
  ...
  async autoClearBooking() {
    try {
      await this.BookingModel.deleteMany({ status: 'reject' });
      return {
        status: 200,
