...
export class BookingController {
  ...
  async create(
    ...
    try {
      ...
      const respone = await this.bookingService.create(
        createBookingDto,
        userId,
      );
  ...
  async acceptBooking(
    ...
    try {
      try {
        const response = await this.bookingService.acceptBooking(
          userId,
          client_id,
          booking_id,
        );
  ...
  async DeleteBooking(
    ...
    try {
      const { userId } = req.user;
      const response = await this.bookingService.DeleteBooking(
        userId,
        client_id,
        booking_id,
      );
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  async userCancel(@Req() req, @Res() res: Response, @Param('id') id: string) {
    try {
      const { userId } = req.user;
  ...
  async sendRequestForClient(@Res() res: Response) {
    try {
      console.log(
        `[${new Date().toISOString()}] Tự động gửi email yêu cầu review lúc 1h sáng...`,
      );

      // Gọi phương thức từ BookingService để lấy danh sách booking cần review
      await this.bookingService.sendEmailsForClient();
      return res.status(200).json({
        message: 'Đã thay đổi trạng thái của user đồng thời gửi request review',
      });
    } catch (error) {
