...
export class UsersController {
  ...
  async update(
    ...
    try {
      ...
      const response = await this.usersService.updateTheoAdmin(
        id,
        updateUserDto,
        userId,
      );
  ...
  async updateMySelf(
    ...
    try {
      const response = await this.usersService.updateTheoUser(
        userId,
        updateUserDto,
      );
      return res.status(response.status).json(response.message);
  ...
  async changeRoleAdmin(
    ...
    try {
      const { userId } = req.user;
      const user = await this.usersService.changeRoleToUser(
        id,
        dto.newRole,
        userId,
      );
