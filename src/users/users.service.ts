...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto) {
    try {
      ...
      const checkEmail = await this.user_model.findOne({
      ...
      const checkPhone = await this.user_model.findOne({
  ...
  async findOne(id: string) {
    try {
      ...
      const bookingUser = await this.BookingModel.find({
        user_id: responseUser?._id,
      }).populate('lawyer_id');
      const reviewUser = await this.ReviewModel.find({
        user_id: responseUser?._id,
      }).populate('lawyer_id');

      const bookingLawyer = await this.BookingModel.find({
        lawyer_id: responseUser?._id,
      }).populate('user_id');
      const reviewLawyer = await this.BookingModel.find({
        lawyer_id: responseUser?._id,
      }).populate('user_id');
  ...
  async changeRoleToUser(id: string, newRole: string, userId: string) {
    const isAdmin = await this.authService.checkAdmin(userId);
    if (isAdmin) {
  ...
  async updateTheoUser(userId: string, updateUserDto: UpdateUserDto) {
    try {
      const { name, phone, role, age, password, avartar_url, province } =
        updateUserDto;
